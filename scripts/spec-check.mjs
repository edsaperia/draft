...
function checkMarks() {
  ...
  if (!live) find('marks', 'the `live` literal (what pins) not found in session.js');
    ...
    for (const r of rows) {
      if (/^if unread/.test(r['pins?'])) continue;
    ...
    // the two news kinds are skipped above because their cells carry the cap;
  ...
  // **The news cap** (Q113, Ed 2026-09-14): three, oldest first, one queue over
  // the band's news entries and the charter's. The table states it in words in
  // three cells; session.js holds it as one named constant and one demotion.
  const cap = sess.match(/const NEWS_PIN_CAP = (\d+);/);
  if (!cap) find('marks', 'NEWS_PIN_CAP not found in session.js — the ✔/✖ green rows say at most three owed decisions pin (Q113)');
  else if (cap[1] !== '3') find('marks', `NEWS_PIN_CAP is ${cap[1]}, the table says the oldest three (Q113)`);
  ...
  for (const r of rows) {
    ...
    if (!/oldest three/.test(r['pins?'])) find('marks', `${r.kind}: the pins cell does not name the cap — session.js caps owed decisions at NEWS_PIN_CAP (Q113)`);
  }
  ...
  // your own can wear is exempt** — ✏️, and since Q170 the stranded ↻ beside
  // it. Named rather than derived, because the literal says *mine* and the
  // table says *kind*, and the mapping between the two is the thing checked.
  const MINE_KINDS = ['propose', 'stranded'];
  for (const r of rows) {
...
function checkSetupAlphabet() {
  ...
  if (!/oldest three/.test(cell('news', 'pins?'))) find('setup-alphabet', `news: the pins cell does not name the cap — layoutQueue pins the oldest three owed decisions over both populations (Q113); the cell says "${cell('news', 'pins?')}"`);
 ...
 *  5. **Neither copy carries a third under the room's number, and both carry
 *     the seconder**, and Appendix A's row says so (Q1439 rulings s and u,
 *     v0.133 → why: R-131, reversing R-073). Half of this claim is about
 *     something *not* being present, which is worth a checker for its own
 *     sake: a ⌈E/3⌉ term put back into either arithmetic would leave the whole
 *     suite green while the card's number stopped being the number the room is
 *     held to, which is exactly the disagreement Q1449 named. `adoptionFloorMax`
 *     is allowed to stay *declared* — logs carry it — so what is asserted is
 *     that neither function's body mentions it or a third of anything. The
 *     other half is `min(2, E)`, the seconder, whose removal the churn study
 *     priced at 888 reversions in 904 adoptions.
 */
