#!/usr/bin/env node
// found-lab — a bot founds the 2026 Governance Laboratory test room on docs.vote,
// seeds Ed's text, invites 14 more bots and two people, settles the constitution
// with three questions delegated, then waits for every answer and presses 🍾
// with every founder power laid down. One-off, 2026-09-15.
import { readFileSync, writeFileSync } from 'node:fs';

const BASE = 'https://docs.vote';
const SLUG = 'lab-2026';
const TITLE = '2026 Governance Laboratory';
const FOUNDER = 'ada.lovelace@bots.docs.vote';
const BOTS = ['alan.turing', 'grace.hopper', 'edsger.dijkstra', 'barbara.liskov', 'donald.knuth',
  'margaret.hamilton', 'claude.shannon', 'hedy.lamarr', 'john.mccarthy', 'radia.perlman',
  'tim.berners-lee', 'frances.allen', 'dennis.ritchie', 'katherine.johnson']
  .map((n) => `${n}@bots.docs.vote`);
const PEOPLE = ['edsaperia@gmail.com', 'marzban98@gmail.com'];
const HERE = new URL('.', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1');
const TEXT = readFileSync(`${HERE}/lab-text.md`, 'utf8').replace(/\r\n/g, '\n').trimEnd();
...
writeFileSync(`${HERE}/lab-founder.cookie`, COOKIE);
...
// 4. invitations — bots first, then the two people
for (const e of [...BOTS, ...PEOPLE]) { await cmd('invite', { email: e }); say(`invited ${e}`); }

// 5. the constitution: held settings set, three delegated
const held = {
  ending: { endsAtMs: Date.UTC(2026, 10, 29, 23, 59) },   // Article 5: 23:59 on 29 November 2026 (London = GMT)
