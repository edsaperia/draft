...
// **38 since Q170** (Ed, 2026-09-14): E38, *a proposal of yours was stranded
// by a text change*, arrived the same day exit 3 stopped being forgiven — and
...
// grow a step for it. E38 was read and no step was written — stranding a
// proposal needs a ground shift this table does not drive, and its audience
// (*the author, and nobody else*) is one seat — so **whether the matrix
// should raise E38 is a question for Ed**, not a step invented here. The
// count is bumped so the rows Q1355 and Q1356 read can report themselves; it
// is not a claim that E38 is covered.
if (EVENTS.length !== 38) {
  shape.push(`SURFACE §2 has ${EVENTS.length} event rows, not the 38 this table was written against`);
}
