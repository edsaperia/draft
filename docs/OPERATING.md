...
4. On `main` only, CI first asks **what the push touched** (Q1347), through
   ...
   of three lanes. The test is on **paths**, not on names: a file is the
   host's unless it is under `docs/`, under `design/`, or is a `.md` file
   at the **top level of the repository**. **Documents only** — every
   changed file is under `docs/` or is a top-level `.md`: nothing is
   deployed. `packages/server/NOTES.md` and `design/MOBILE.md` are not
   documents by this filter, being neither at the top level nor under
   `docs/`: a push touching only the first takes the full, paused lane
   below, and one touching only the second takes the surface lane.
   **The surface only** (every changed file is a document by that rule or
   is under `design/`, and at least one is under `design/`): the served
...
answers `{ ok, build, surface, catalogue, store, documents,
documentsSkipped, documentsQuarantined, documentsStalled, paused,
uptimeSeconds, mail, devMail, outbox, errors, cooldownMs }`. Issue #8 adds
`booted`, the commit the process booted with, beside the `build` it serves.
The five that are not self-explanatory: `surface` is the commit a surface
upload put in place, or `null` — §3's *Restarting the live host* on why a
`null` there after a restart is not nothing; `documentsSkipped` counts logs
below schema version 2 and is 0 on docs.vote since the wipe (§5);
`documentsQuarantined` counts logs whose replay threw (§5 point 1);
`documentsStalled` counts documents the store has refused for good (§11);
and `paused` is the announced pause's payload, or `null`. It is the one
route excluded from the access log, so a health check every few seconds does
not drown it. `devMail` is
