...
TypeScript end to end; `pg` is the only runtime dependency. Tests measured 2026-09-11 with `npm test`: **1,142 passing** (9 todo, 17 skipped without Postgres).
...
| `packages/constitution` | The document's rules as a module, equally pure: the settings catalogue, the blind founding (each member states the least they will accept; the document takes the maximum), motions on both routes — ordinary ones race, constitutional ones need everybody — applications, lapse, its own hash-chained log. Runs in the browser too, as the committed bundle `design/constitution.js`. Notes: [`NOTES.md`](packages/constitution/NOTES.md). | 700 |
| `packages/server` | The product host: `node:http` with no framework, one hash-chained log per document as the only persistence (JSONL on disk or one row per entry in Postgres), magic-link login, stateless HMAC cookies, the engine riding every commit. Notes: [`NOTES.md`](packages/server/NOTES.md). | 103 (+17 Postgres) |
...
|---|---|---|
| [`SPEC.md`](SPEC.md) | The mechanism, v0.115 — tables and numbered rules, each pointing at its reasons as `→ why: R-nnn`. The single source of truth. | First, to understand what the engine does. |
