...
- **[SPEC.md](SPEC.md)** — the specification (currently v0.78), single source of truth for the mechanism.
...
```
npm test               # every workspace (898 tests)
npm run typecheck
npm run lint
...
Spec v0.78. The mechanism (engine-core, 249 tests) and the constitutional layer (553 tests) are built and tested; the server (65 tests, 14 more against Postgres) hosts real documents at docs.vote; the simulator (31 tests) is what keeps all of it honest.
