...
- **[SPEC.md](SPEC.md)** — the specification (currently v0.76), single source of truth for the mechanism.
...
```
npm test               # every workspace (531 tests)
npm run typecheck
npm run lint
...
Spec v0.76. The mechanism (engine-core, 222 tests) and the constitutional layer (234 tests) are built and tested; the server (44 tests, 14 more against Postgres) hosts real documents at docs.vote; the simulator (31 tests) is what keeps all of it honest.
