...
TypeScript end to end; `pg` is the only runtime dependency. Tests measured 2026-09-18 with `npm test`: **1,340 passing** (9 todo, 18 skipped without Postgres).

| Package | What it is | Tests |
|---|---|---|
| `packages/engine-core` | The mechanism as a pure, deterministic, dependency-free library: diffs and footprints, the races, Bradley–Terry ranking with ties, the session state machine, the hash-chained event log, the feed router, and the participant API — the one blind surface that people, simulated members and personal AIs all speak identically. Notes: [`NOTES.md`](packages/engine-core/NOTES.md). | 402 |
