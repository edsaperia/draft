...
- **[SPEC.md](SPEC.md)** — the specification (currently v0.90), single source of truth for the mechanism.
...
```
npm test               # every workspace (1055 tests)
npm run typecheck
npm run lint
...
Spec v0.90. The mechanism (engine-core, 277 tests) and the constitutional layer (670 tests) are built and tested; the server (99 tests, 14 more against Postgres) hosts real documents at docs.vote; the simulator (31 tests) is what keeps all of it honest.
