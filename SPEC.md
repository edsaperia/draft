...
The cooldown is a legibility device, not a quality device — it is a *recognise-decisions* cadence, and it is **engine tuning, never a constitutional setting**. It must stay short (≤5 min). A cleared race is **not surfaced as waiting** — no ⏳: the batch evaluates fresh at its own moment, and a "cleared and cooling" state would be a standings reveal (§3.5). The cooldown is the one brake on the pace of change: it backs the floor, and nothing else does. → why: R-116 **Its home is the operator, not the document**: set once for the host (`DRAFT_COOLDOWN_MS`), the same for every document it serves — not in the catalogue, not in the record, not a motion anybody can raise. **The host's default is no cooldown**: unset, adoptions land the moment they clear, and the variable switches pacing on. A document begun under another value is re-paced to the host's at its next sweep, by an amendment in its own log. → why: R-052, R-074, R-086
