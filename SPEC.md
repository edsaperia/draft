...
One comparison is recorded without being asked for — *specified here, deliberately not yet in engine-core, pending Q245*: the author's own draft against **the text it was written against** — the status quo, or the specific candidate they pressed *propose edit* on. Assuming that preference saves a pointless question; assuming any other would be fabrication. This judgment counts toward the race's floor (§8.2): an author is a voice on the roster, and excluding them would systematically under-count the preferences of the people who cared enough to write.
