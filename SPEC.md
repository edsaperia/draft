# Group Drafting Engine — Specification v0.108
...
**E is the arrived, non-removed, non-lapsed membership** (Ed, 2026-08-18) — one definition, two uses: quorum's share form is ⌈share × E⌉ (§4.2) and the adoption-floor term is ⌈E/3⌉. An invited member who has never arrived counts toward nothing (§9.6a — membership begins at first arrival), and a lapsed member has already left the base (§9.5a). Silence is never imputed (Q43, Ed 2026-08-14 → why: R-089): a race short of the floor simply waits — the unheard boost keeps asking, and at close it degrades gracefully to backlog, the session never adopting around a legitimate but silent member. The system may surface sustained inactivity to the convenor as an advisory signal; what removal takes depends on whether the document has started (§9.6a), and F recomputes from the new E.
...
**9.0c Identity is not authorship.** Each member chooses a display name and a picture, settled by them alone — personal settings: not part of the constitution, not delegable, **offered at the founding and never required**, because they bind nobody — neither card holds the start (§9.0b). → why: R-079 **Anonymous is chosen, never inferred** (Ed, 2026-09-02, Q1164, v0.96): each card offers *Anonymous* as its own answer, a blank field is no answer at all, and a member who has answered nothing keeps the default appearance until they do. A member who chooses it is shown as **Anonymous** — a name, not a gap. → why: R-084 A name is how you appear **in the room**; authorship is whether a name is attached to a **proposal**, which §3.5a governs. → why: R-036 A **picture** is an emoji, an uploaded image, or none — with none the member shows as their initials, or as an anonymous mark before they have a name. An emoji is worn by one member at a time, and may not be one of the glyphs the surface uses for its own vocabulary. → why: R-046
...
| machines | 🤖 | ordinary | yes | no | — | machines | — | most machine proposing accepted | ordinary | any time — surface: none, retired 2026-08-29, see R-078; delegated before then, 🍾 resolves it at {enabled:false, budget:0} (R-080) |
...
| Post-adoption cooldown | none unless the operator sets one (`DRAFT_COOLDOWN_MS`, ≤5 min — Q946, R-086); engine tuning, never constitutional (§4.2) |
| Redraft limit before carry | 2 informed redrafts |
| Rendering tiebreak | deterministic (hash order); margins in the record |
| Stake | 1 ✏️ per candidate, flat (§7, §13); the drip interval is the room's (⏱️, §9.7.1); grant and cap are 3 (§7, R-083) |
