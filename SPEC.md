# Group Drafting Engine — Specification v0.23
...
Two consequences. A deadlocked race that a participant has not judged **counts as work to do** for the empty-queue test in §8.3a, so it defers the diagonal. And the **bounty board is not gated by this rule** — it is public and names every deadlocked race, because going to the board is a deliberate act of looking for stuck work and a board that hid it would have nothing on it. The board says where the room is stuck; the card still asks the individual for their judgment first. (Q299 records the alternatives.)
...
| Deadlock disclosure (§8.3b) | per-participant: shown as an ordinary race until it has no pair left to serve that participant; bounty board ungated |
