# Group Drafting Engine — Specification v0.133
...
**4.5 The certification gap — retired at v0.128.** The gap was the distance between a race's certification and a rising bar; with no bar (§4.2) there is no gap. What it did — adoptions made on noisy early evidence self-correcting as the room keeps judging — the ranking does by itself: a race is never closed (§4.4), so a later majority the other way puts the current text back on top. → why: R-117
