# Group Drafting Engine — Specification v0.65
...
**9.0a The founding.** Every setting is born convenor-held, both powers, its question shut and its value unset: nothing arrives delegated and nothing arrives answered. → why: R-001. Before the start the convenor sets what they hold freely, and delegates what they choose; delegating a delegable setting opens its **founding question**, answered **blind** by the arrived members:
...
**9.5a Lapsing.** Whether a membership **lapses** after a long quiet spell — and after how long — is a constitutional setting (💤): each member states the **shortest** quiet spell they would accept and the document takes the longest, *never* being the longest of all. A lapsed member leaves the quorum base the way an abstainer does; judgments already cast keep counting; warnings go by mail before it happens, and the **package** — the document as it stands and the record to date — goes out with the lapse. Returning is logging in again: the rule was consented to at the founding, so revival needs no motion. The convenor's clock runs too (§9.7 rule 6). → why: R-037
...
3. **Relinquishing a power is free, separate and one-way after the start.** Before the start both powers are as revisable as any value (`reclaim`). Laying down 🛡️ alone is available from creation; laying down ✒️ alone waits for the start, because the shield-only state is inert before it. **Laying a power down is news**: every member is owed an acknowledgement of it, as of a changed rule. → why: R-007, R-044
...
| removal | 🚪 | constitutional | yes | no | — | ladder | everyone · others · ordinary | easiest removal accepted | constitutional | any time |
| machines | 🤖 | ordinary | yes | no | — | machines | — | most machine proposing accepted | ordinary | any time |
| membership | 🪪 | constitutional | no | no | — | register | — | — | commands (X4) | through 🤝 |
| applications | 🤝 | constitutional | yes | no | — | applications | — | most open policy accepted, over invite · proposed · apply · open | constitutional | any time |
...
| X4 | The register is changed by commands, never by a value; its powers are 🤝's | rule 5 | invite / arrive / remove are acts on people | §9.7½, Q506 |
| X5 | 🚪 *others* is a decision class of its own: unanimity minus the subject, who sees the motion running but is not asked; a rung change mid-motion is a ground shift | routes | real constitutions expel by unanimity of the others | Q401(a) |
...
| X11 | Under *apply* the applicant authors their own admit race, a voice for that one act, **and their derived preference is not a mover** toward its floor | §3.3, §8.2 | their application is a type of proposal; a wallet-less voice must not be its own floor | Q397, Q565 |
...
**9.7½ Applicants.** The membership question is two settings: 🪪 **Membership** — the register, changed by command (invite, arrive, remove) — and 🤝 **Applications**, whose value is the **join policy** and whose power pair is the register's: *invitation only* · *applications must be proposed* (an application waits until a member proposes it, staking an ✏️ — a second) · *anyone may apply* (straight to the threshold, free) · *open* (anyone with the link joins on arrival). → why: R-024. Where application is allowed, a reader who is not a member may apply: an **email, verified by magic link before anything is submitted** — the applicant's identity; **member emails are unique** (an address already on the membership is told to log in instead) — then a name, a picture and a few optional words; an empty application is a real application. A submitted application is **an ordinary motion to grant membership**, deliberately a different door from invitation: each application is its **own one-candidate race against the membership as it stands** (*admit them* against *keep the membership as it is*), carrying at the threshold with quorum; two applicants are never raced against each other. The candidate's author is whoever proposed it — the seconder under *proposed*, the applicant under *apply* (X11). **Every proposal to bring somebody in offers a rationale, and a blank one is a real proposal.** An admitted applicant inherits the constitution and is owed its acknowledgements like any late joiner; either way they are told by mail.
...
| Post-adoption cooldown | 5 min; engine tuning, never constitutional (§4.2) |
