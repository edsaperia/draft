# Group Drafting Engine — Specification v0.110
...
**9.5a Lapsing.** Whether a membership **lapses** after a long quiet spell — and after how long — is a constitutional setting (💤): each member states the **shortest** quiet spell they would accept and the document takes the longest, *never* being the longest of all. A lapsed member is outside E and so **stands outside every electorate** — a 🏛️ does not wait on them, and they are counted as abstaining in every decision meanwhile; judgments already cast keep counting; warnings go by mail before it happens, and the **package** — the document as it stands and the record to date — goes out with the lapse. **Returning is being here again, and nothing else** — a read of the document, a login or an act, whichever comes first: no price is paid twice, the rule was consented to at the founding, so revival needs no motion, and a lapsed member is never shown the room they are not counted in. The convenor's clock runs too (§9.7 rule 6). **Lapse is a reading of the rule, re-read when the rule changes**: when 💤 is turned off, or lengthened past their quiet, a lapsed member — and a lapsed crown — is returned at once; a shorter spell moves nobody until the clock does. → why: R-037, R-095, R-096
