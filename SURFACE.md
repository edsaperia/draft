...
- **F5 🍾 is the founder's, invisible to members before the start, and it does not appear until it can be pressed** (F9): shown once the module's own `readiness()` is ready and 🏛️ is not still being served, hidden before that rather than greyed. Its card states the whole batch and the readiness readout, which informs and never blocks. → why: the readout's *except while a judge-gate question is still collecting* was outgrown twice — R-045/Q626 widened `waitingOn` past the judge-gates, and Q745 stopped the card appearing at all before then.
