/**
 * The doors a person arrives through (refactor Q1352 (m)): the address
 ...
 *
 * One family, in the chain's order, because every branch here ends in a
 * token, a cookie or a refusal: nothing in it reads a document as a member.
 */
...
export async function authRoutes(ctx: RouteContext, r: Req): Promise<boolean> {
  const { req, res, url, path, seg, nowMs } = r;
  const { cfg, store, stash, auth, mailer, writes, commits } = ctx;
