...
import { NO_TARGET } from './demo-target.js';
...
export async function createDraftServer(cfg: ServerConfig,
   ...
   *
   * **The target is the demo document** once Stage 1's `Demo` is on this
   * branch (`demoTargetOf(demo)`, demo-target.ts); until then a server's bots
   * have no document unless a test or the dev route names one.
   */
  ...
  const demoBots = new DemoBots({
    ...
    target: options.demoTarget ?? NO_TARGET,
  ...
  const ctx: RouteContext = {
    ...
    errors, races, demoBots, bootedAtMs, httpsOn,
  ...
  return { server, store, auth, mailer, outbox, demoBots, tick, close };
}
