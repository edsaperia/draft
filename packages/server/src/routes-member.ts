/**
 * The member surface (refactor Q1352 (m)): `view` — the only read a member
 * is ever served (§3.5) — `cmd`, the one write, and the founder's pre-confirm
 * text stash.
 *
 * **The stranger's door is inside this family, not beside it.** It is not a
 * branch of its own: it is what the view branch answers when the request
 ...
 * belongs.
 *
 * The view/cmd branch is also the one branch that can **decline**: a living
 * seat asking for anything but `GET view` or `POST cmd` falls through the
 * block and on down the chain, having written nothing. That is preserved
 * here as a `false` return.
 */
...
export async function memberRoutes(ctx: RouteContext, r: Req): Promise<boolean> {
