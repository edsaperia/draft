 ...
 * panel's tab is open. Ed-only: every row asks `demoGate` (`demo-key.ts`), so
 * with no `DRAFT_DEMO_KEY` on the host each is a 404 byte-identical to an
 * unknown path's, and without a valid `draft_demo` cookie a 401 behind the
 * per-IP limiter, as `bearerRefused` answers. No row takes a document id or a
 ...
 */
import { demoGate } from './demo-key.js';
...
/** The gate every row asks: true when it has answered the request itself. */
function refused(ctx: RouteContext, r: Req): boolean {
  const gate = demoGate(r.req, ctx.cfg.demoKey, r.nowMs);
  if (gate === 'off') { json(r.res, 404, { error: 'not found' }); return true; }
  if (gate === 'refused') {
    if (!r.tooMany('demo')) json(r.res, 401, { error: 'unauthorized' });
    return true;
  }
  // a POST from another site must not drive Ed's bots on his cookie
