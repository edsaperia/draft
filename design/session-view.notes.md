...
**Urgency is saturation and size** (104, 105). Colour saturation carries how much an entry wants you — a wash of the primary from 5% to 30% alpha — and the amount of card drawn falls with it: **hot** keeps its label, kind chip and caption; **warm** drops the caption; **cool** is a single line.

The first build of this made the tint a flat fill and so quietly deleted the **progress wash** — the left-to-right gradient that had always shown closeness to resolution across the whole rail. That was a loss I introduced without flagging it (Ed caught it, 116). It is back, and the two magnitudes now share one device rather than competing for two: **how far the fill reaches** is closeness, **how strong the colour is** is urgency. A saturated race still washes grey rather than blue, so *more judgment won't move this* stays visible as its own thing (SPEC §8.3). Worth watching whether one device carrying two numbers reads cleanly; the fallback is a separate hairline `evidence-meter` on each card, at the cost of more ink.
...
**When it doesn't fit**, the rail keeps the entries nearest your reading position and counts the rest — *+N further off in the charter*, pinned at the foot of the band. Strictly nearest-first, so the visible set is one unbroken run rather than an assortment of whichever cards happened to be small. Nothing is ever dropped silently.
...
Two consequences for how a saturated entry is drawn.

**It stops being grey.** It had been washing grey along with the closed states, which under 164's principle says "nothing here is yours to do" — the exact opposite of the truth. A saturated race is arguably the most actionable thing in the rail. It now carries the open yellow at a fixed strength, so colour still means *you can act* and the glyph carries what is different about the acting.

**And it stops riding the urgency ramp.** Urgency is judgment leverage, which for a saturated race is zero — so it was landing at the bottom tier and compressing to a single line, hiding the one entry whose instructions differ from every other. It now has its own tier, drawn full, with its caption promoted from a progress note to an instruction: *saturated — more judgments won't move it; only a new proposal can*. A saturated race is not a low-urgency race, it is a differently addressed one.
