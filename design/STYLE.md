...
| T6 | A power option is the clause sentence the head writes, held or given, with the negation bold (Q1378, Ed 2026-09-15: *option text is not document text* — the paraphrases and the veto sentence left the blocks); `vetoLabel`'s setting-naming sentence survives where no head names the setting — the rail, the deck line, the record (`pwSaid`). **On the decision card the sentence names its setting** — *amend the title at will*, *refuse changes to the title that the membership pass* — because the card replaces the paragraph that gave *this* its referent; in the document the clause keeps *this* (Q1429, Ed 2026-09-17) | `pwOptText`, `pwLine`, `powerHeadLine`'s `named`, `vetoLabel`, `pwSaid` |
