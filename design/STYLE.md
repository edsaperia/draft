...
| `[redacted]` (for a person whose row was erased) | withdrawn, an erased member, the anonymous disc alone | Ed, 2026-09-08, Q1287 (b): an erasure strikes the record; it does not report a departure. The one bracketed string on the surface — T49. |
...
| T6 | A power option is the clause sentence the head writes, held or given, with the negation bold (Q1378, Ed 2026-09-15: *option text is not document text* — the paraphrases and the veto sentence left the blocks); `vetoLabel`'s setting-naming sentence survives where no head names the setting — the rail, the deck line, the record (`pwSaid`) | `pwOptText`, `pwLine`, `vetoLabel`, `pwSaid` |
