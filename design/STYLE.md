...
§8 numbers them (T1–T49) for citation and says where each is enforced.
...
| **approval threshold** | the bar, θ, confidence, comparison, Bradley–Terry, **and *threshold* itself** | **Engine vocabulary, with no site on the surface at all** (Q1362, Ed 2026-09-15). The threshold left the mechanism a member meets: the document's text on a footprint is whichever wording the ranking puts on top once the quorum is met, so there is no bar to name, explain or link to. `/pairwise` and 🌡️'s method note — the one sanctioned exception, entry 163 — went with it. Guard: `spec-check`'s `BANNED`. |
...
| `[redacted]` (for a person whose row was erased) | withdrawn, an erased member, the anonymous disc alone | Ed, 2026-09-08, Q1287 (b): an erasure strikes the record; it does not report a departure. One of the two bracketed strings on the surface — T49. |
...
- **T18 · Decided is a word, not a glyph**: *OK* on anything that only wants
  to have been seen — there is no glyph in common use for *I have taken this
  in*. **A grant is the exception since Q1501** (Ed, 2026-09-22): it is
  accepted, not seen, so its commit reads *Accept ‹glyph›* — 🏛️'s too since
  Q1541.24, *Accept 🏛️* — and **the gates 💡 ⚖️ take the same exception**,
  *Accept ✏️* · *Accept ⚖️*, being drawn like grants (answers.md Part 4
  .24). Once accepted the card asks nothing and has no row at all: the
  close-only *OK* is gone (1541.8 (a), Q1541 stage 2).
...
| T6 | A power option is the clause sentence `pwLine` writes, held or given, with the negation bold (Q1378, Ed 2026-09-15: *option text is not document text* — the paraphrases and the veto sentence left the blocks); at the founding the card has no head and the two blocks are the whole choice, and once the document has begun the head states the rule and the held option is not offered back (Q1430, Ed 2026-09-17); `vetoLabel`'s setting-naming sentence survives where no head names the setting — the rail, the deck line, the record (`pwSaid`). **On the decision card the sentence names its setting** — *amend the title at will*, *refuse changes to the title that the membership pass* — because the card replaces the paragraph that gave *this* its referent; in the document the clause keeps *this* (Q1429, Ed 2026-09-17) | `pwOptText`, `pwLine`, `powerHeadLine`'s `named`, `vetoLabel`, `pwSaid` |
...
| T18 | §5 · Decided is a word, not a glyph — OK on anything that only wants to have been seen; a grant or gate not yet accepted reads *Accept ‹glyph›* (Q1501, Q1541.24) | `data-seen`, `data-ok`; `grants`, and no `take` |
...
| T28a | 🖼️'s version of T28: the card says nothing about no picture (Q1165 took the sentence off; *Anonymous* is the first block), and the uploader says why a picture was turned away, in its own box, only when one is (2026-09-17) | `picNote`: *That is not a picture.* · *That picture is too big to open here.* · *That picture could not be opened.* · *That picture will not compress small enough — try a simpler one.* |
...
| T39 | ~~A **meaning** names its own dependence — the membership, the window, the quorum — and no other: it lives on the card, never in the clause, states one consequence under H4's budget, and where there is nothing true to say the card prints no line rather than a stand-in of its own (entry 167)~~ — **retired 2026-09-18** (Ed, Q1439 (u): *please remove the meaning line* — all three): no card carries a meaning line; the rule sentence stands alone, its numbers inside it (👥's *(x of y)*, ruling m). `meaningOf` survives engine-side under `meaning.test.ts`. **One exception since Q1490** (Ed, 2026-09-21 → why: R-139), and no other card may take it: 👥's scale runs 1–100, and at either end of it the number does not say what it comes to, so the card adds **one** sentence — above half, how few members preferring the current text can stop a proposal; below two, that the minimum quorum is 2, the author and one other member. One at a time, never both, nothing for a number nobody has typed and nothing in a membership of one. `quorumStop` / `quorumFloorMin` in `design/copy.js`; `quorumNote` in `design/setup.js` is the only chooser | 👥: *With this quorum, 6 members preferring the current text can stop a proposal.* · *The minimum quorum is 2: the author and one other member.* |
