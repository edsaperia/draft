...
Three hold sites today, each owning its own `pointerdown` → timer → fire body: the charter ✏️ (`session.js:2744`), `holdWallet`'s family (`session-view.html:6682–6818`, 🪶 ✒️ 🍾 and the motion ✏️ alike), the assembly — every one of them `SESSION.holdMs`, 1000ms, since backlog 206. The fire bodies are the valuable part and are already id-resolving. Extract each into a named `fire()` and route the *gesture* through **`SESSION.press(ev, spec)`**, `spec = { ms, floorAt, fire, flight: {start, stop, nudgeHome}, arm: {lean, unlean}, key }`:
