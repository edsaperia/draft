...
- **At rest it peeks** (`data-sheet="peek"` on the root): a 56px bar at the window's foot above the home indicator's inset — a grab handle, the most urgent entry's own title line (its `.ql`, cloned from the entry as the rail drew it, so the bar and the entry never say different things) and *+n more* (`session.rail.sheetMore`). The population and its order are the drawer's (`layoutQueue`'s narrow branch, Q1351), which announces what it laid as `tasks-laid`; so the open entry names itself while its card is open, and when the card is closed or answered the bar names the next most urgent. **Gone entirely while nothing asks you** (no `data-sheet`); the ≣ door's count and red are unchanged.
