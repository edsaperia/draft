...
1. **Scope of the first mobile version: read + judge.** Read the document and the constitution; act on every rail card — judgments, founding answers, OKs, applications, sign-out. Proposing text (the composer), running a founding or a birth, and composing motions come later — but the architecture must make each a slot-in, never a re-layout (§1.7).
...
- **Topbar** — stays sticky (the sockets say what your role can do, W11). One row does not fit (🪶 ~40 + brand + room line ~120 + four sockets ~160 + `me` 26). **The brand row keeps 🪶, the title (ellipsis, `.sess` hidden), a ☰ contents control and `me`; the room line (pulse · clock · quorum) and the four sockets drop to a second row inside the same sticky nav** (~84px). `--nav-h` is load-bearing, so rather than a second token **`BAND_TOP` is measured** (`navEl.offsetHeight + 12`) per layout; on wide 58 measured = 58 declared. (Q656.)
...
SURFACE §7.2's holds table gains an **on touch** column (*two taps; the flight announces* on every hold row, *—* on OK) plus an exception row in §3, and `spec-check` asserts exactly three sites call `SESSION.press`. **In a read + judge v1 the only hold a phone meets is the founder's 🪶 at `/`** — judging is a click — so the birth stays reachable-and-unsupported on phones to give the gesture one live exercise (Q663).
...
**Q429 budget**: 1 MB uncompressed / 250 KB transferred, enforced in `spec-check.mjs` by summing the precache list read from `sw.js`. Current estimate ~600–800 KB uncompressed: headroom, not much.
...
- **Sending**: hand-rolled `packages/server/src/push.ts` on `node:crypto` (P-256 ECDH, HKDF, AES-128-GCM, RFC 8188 `aes128gcm`, RFC 8292 VAPID — ~150 lines) with RFC 8291's test vector as the unit test, keeping the server's zero external deps; the alternative is the `web-push` package and its tree (Q670).
...
- **What push carries**: SURFACE §2's channel column gains `push`. Proposed **C17: push carries asks and owed OKs, never standings** (§3.5) — payload `{slug, title, kind}`, the body is the rail entry's own sentence. Rows gaining `push`: E4, E5/E7, E10, E13 (served, so the router decides who), E14, E17, E23, E24. Mail-only: E22 lapse (reaching the absent is the point), E25/E26 and invitations (the magic link is the login; a push cannot carry a seat). **E27 never**: off the page the beat *rate* becomes legible as activity — the leak the dot avoids by being where you already are. Coalesced on the server's minute tick: one notification per member per document per tick, *"<title>: n things want you"* (n = your rail length, never standings). Hook: `relay()` (`server.ts` ~113) as a second queue beside mail; engine-log events (E13/E14/E17) need the same relay over the bridge's fresh entries. (Q672.)
...
### 3.f Q423 — the pulse's third state: yes, but not on the dot

Silence and stillness are different facts and must not read alike; but the dot is grey at rest and at its brightest and beats only by growing a ring — it cannot carry a third state within its grammar. Offline/stale: the dot `visibility: hidden` (slot kept, like a ghosted socket) and the clock line carries *as of HH:MM*. On `visibilitychange` → visible the page treats itself stale until the first poll answers, then the dot returns and beats once if anything moved.

## 4. Recommended answers to Q419–429 (Ed accepts or vetoes by number)
...
- **423** Third state on the clock line, dot hidden, never a colour (§3.f).
...
- **429** 1 MB / 250 KB, enforced in `spec-check` (§3.c).

## 5. Decisions 655–673 (recommendation first; open in QUESTIONS.md until answered)
