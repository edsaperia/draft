...
- **Six dormant `max-width: 900px` rules** in `design/system.css` (lines re-checked 2026-09-07: :358 two-column at 1240px, :362 one column, :870 `.doc` padding, :940 `.sectoggle` static, :1145 `.chipcol` → horizontal row, :2091 `#wires` hidden). A first attempt, partly wrong: the `.chipcol` row breaks `fitStacks` (it measures a vertical gutter) and `fitBand` (which filters neighbouring marks by `|left − left| < 20`, `setup.js:514`); `layoutQueue` (`session.js:1014`) writes absolute `top`s from the rail's own rect, and `aside.queue` sits *below* `main` in the DOM, so in one column every entry lands at a large negative offset. `.queue` is `position: relative`, not sticky — `session.js:5211`'s comment is stale.
- **Touch**: `touch-action: none` exists on exactly one element (`.holdmotion`, the 🏛️ button). The propose hold (`session.js:3175`, `HOLD_MS` — one second since backlog 206) and the wallet holds (`session-view.html:11475` onward, `penHold`) would be scrolled away and `pointercancel`led.
- **Hover-only**: the fold triangle (`system.css:414`), the spend-preview lean (`session.js:4926`, `applyLean`), `.queue button:hover`, `.achip:hover`, `.doctitle[data-card]:hover`, and ~66 native `title=` tooltips across the four files. `.walletsay` dismisses on `resize`, which a mobile keyboard opening fires.
- **Editing**: one `contenteditable` host for the whole charter (`session.js:203`, `PROSE`); `startDraftFromTyping` (`session.js:2273`) switches on `inputType` and `default: return`s — `insertCompositionText` (Android keyboards' ordinary typing) opens no draft. Out of v1 scope; the seam is §1.7.
...
- **Server**: static files by basename from `design/` (`server.ts:1560–1610`); `serveFile` (:1827) sets no `Cache-Control`, ETag or compression; `x-build: <sha>` is already sent on every response (the asset-versioning seed a service worker needs); `/api/*` is `no-store`. No service worker, manifest, VAPID or push anywhere; notifications are email only (`relay()`, `server.ts:199`). The `?since=` poll already answers *did anything move* content-free.
- **Decision 436's `people` table is adopted but unbuilt** (schema v1: `documents`, `document_log`, `engine_log`, `provisional`, `bridge_state`, `tokens`, `stashes`; migrations 2–4 since add `stashes.slug`, `stashes.doc_id` and `outbox`, still no `people` — re-checked 2026-09-07), so Q428's target does not exist yet.
...
- **Built:** the viewport meta (§1.8); `NARROW_Q` in session.js and the same literal in system.css, **asserted by spec-check since 2026-09-14** (`checkNarrow`, with Q1352 (j)'s tags); one column below 900 with **both rails as drawers** (Ed, 2026-09-12, revising decision 8's one drawer and §1.2's dock: *sidebars that slide out on each side, opening with icons in the top left and top right*) — ☰ at the top left slides the contents rail in from the left, ≣ with the task count at the top right slides the task rail in from the right, the ground darkens, a tap outside or Escape closes, choosing inside closes after the choice lands; the task rail lists every entry with a clause in document order (`layoutQueue`'s narrow branch positions nothing, `drawWires` draws nothing, §1.3); the gutter a 56px column with the two flattening rules deleted (§1.9); the topbar without sockets, faces, pulse **or the quorum reading** (§1.2 — the last a first-cut cost: the title had two dots left); `#ridetab`, `#editdoor`, `#prosectl` and the lane's *propose edit* not drawn, so there is no way into the composer; the alpha flag a strip under the topbar; `touch-action` and the tap-highlight and callout rules on controls (§1.5). The drawer script is self-contained in session-view.html under the header; nothing else on the page knows a drawer exists.
- **Not built:** the pinned/flow split (§1.2 — every entry is listed), the two-tap confirm (§1.6 — a hold works on touch by `touch-action: none` alone), tap targets (§1.4), the `:hover` wraps and the `say` long-press (§1.5), the keyboard and `dvh` rules (§1.8), `--border-strong` (§1.2), `mobile-walk` and the narrow probe (§2); the SURFACE.md rows these rules should become.
- **Measured** (iPhone 13 and Pixel 7 contexts, session and founding fixtures): scale 1, no horizontal overflow at load or with a card open; ☰ opens the contents drawer and a heading travels and closes it; ≣ opens the task drawer, an entry opens its card in the document and closes the drawer; a tap on the ground closes; a lane tap arms ✓; no page errors. Both probes IDENTICAL at 1600×1000 after a re-freeze for the two door buttons in the topbar.

### The same afternoon's pass (Q1350–Q1351, Ed 2026-09-12)

Four rulings, asked one at a time, and what each became. Where this list and §1 disagree, this list is what is built and §1 is the older plan.

- **The tab meets the card** (*tabs overlap open cards too much*): the first cut's 8px gutter margin (`.chipcol`, was 14) left the open card's edge — placed for 14 — 6px under the tab on every card. The narrow rule is deleted; the column is absolute, so 14 costs the prose nothing. Measured 0px on the charter and founding cards at 390. **Guard: `npm run card-audit:narrow`**, the audit at 390×844 — run `npm run card-audit` first and pass `--baseline=design/tools/card-audit.json` to keep only what both sizes see; the D1 door step skips itself below 900 (no composer on a phone).
- **The drawer holds only what asks you** (§1.2's pinned population, the dock's rule in the drawer): 🔥, an unread ✔✖ owed to you, ✏️ yours, 🌶️ served, plus the open entry, most urgent first (`layoutQueue`'s narrow branch, `order` on a flex column); the filed population belongs to the gutter tabs alone. The door's number is the drawer's length, **in a red circle while it is more than nothing** (`--slash`, the surface's one red) and grey at zero. On the session fixture: 47 rows → 8.
- **The topbar takes two rows** (*we could have the topbar with two rows; then it can fit everything*): the top row is the document — clock · title · the powers as bare glyphs, 🪶 among them (*quill is a power, it goes with the other powers*); the bottom row is the people — ☰ without its box · quorum · faces · you · the task door. Every element is the desktop bar's, placed by `order` with the wrapping boxes as `display: contents` and the row break a `::after`; **the markup is unchanged and both probes stay IDENTICAL at 1600**. The ✏️ wallet and the quill take a phone form — one glyph and its number (`renderWallet`, `renderPowerWallets`) — and ✒️ 🛡️ drop their ∞. The bar is 83px at 390 and **`--nav-h` is written from the bar's own height on narrow** by the drawer script, so the drawers, the ground and the reading line follow it. The title's mirror at the document head (the morning's ruling) went with this. **Cost, measured: the title gets 118px at 390** on the session fixture (113 on the founding page) — *The Hollow O…* — because the clock and five powers take the rest; a question for Ed if it bites.
...
### The task sheet (Ed 2026-09-24)

Ed's ruling: the task drawer that slid in from the right becomes a **bottom sheet that peeks** — the Apple Maps and Spotify mini-player pattern — so a member always knows their tasks are there without the list stopping them reading. Where this and the first cut's *both rails are drawers* disagree, this is what is built; the contents rail is still the left drawer. §1.2's dock, never built, is what this most resembles, but it lists one entry at rest, not 40% of the window's worth.

- **At rest it peeks** (`data-sheet="peek"` on the root): a 64px bar at the window's foot above the home indicator's inset — a grab handle, the most urgent entry's **own card** and *+n more* (`session.rail.sheetMore`). The card is the entry's button cloned with its wash attributes (Ed, 2026-09-26: *when it's closed you should see the most urgent card, and it should have its colour wash progress bar*), so the rail's own rules draw its hue, ground and progress fill, `settleWashes` fades it with the original, and the bar and the entry never say or show different things; in the peek it keeps its title row alone, so every kind of entry fits (56px became 64 for it: a 37px card stood 2px off the glass). **A tap on the card opens that entry's card**, as a tap on it in the list does, and the sheet stays at the peek; a tap on the rest of the bar (*+n more*, the grab strip), a swipe from anywhere on the bar, the card included, or ≣ raises the sheet. The population and its order are the drawer's (`layoutQueue`'s narrow branch, Q1351), which announces what it laid as `tasks-laid`; so the open entry names itself while its card is open, and when the card is closed or answered the bar names the next most urgent. **Gone entirely while nothing asks you** (no `data-sheet`); the ≣ door's count and red are unchanged.
- **Raised** (`open`): a tap on the bar, a swipe up on it or the ≣ door lifts the whole list, at most 74% of the window (`dvh` where the browser has it), scrolling inside itself with `overscroll-behavior: contain`, over the darkened ground. A tap on the ground, a tap on the list's empty space (Q1388's rule, kept), a swipe down, Escape or the door lowers it; tapping an entry opens its card in the document as the drawer did and lowers it a beat later.
- **The drag follows the finger** and settles to the nearer rest when let go, or to the one it was flung towards (faster than 0.4px/ms over the last 100ms). A tap is told from a drag by 8px of travel, the drawer's rule. **A finger on it is a press in flight** (`data-sheetdrag`, read by `pressInFlight`), so the 4 s poll waits; every state lives on the root, never in the list, so a rail rebuild resets nothing.
...
- **The constitution band** — unchanged in structure (it already shares the charter's gutter geometry, `setup.css:704`; `fitBand` is measured in both axes). Needs only the gutter at a narrower width; the open `.setupcard`'s `margin-left: -14px` and the active tab's 8px outward growth set the gutter's minimum.
...
- **Q421 (fold triangle): always visible under `(hover: none)`, hover-revealed on hover devices; one rule, two renderings** (Q658). The rule (`system.css:414`) is *only there when you reach for it*: on a hover device reaching is hovering, on touch there is no reaching, so the triangle stands. Making it permanent on desktop adds a glyph to every heading to serve a device that is not in the room. `:940`'s `static` rule is deleted — the triangle stays in the gutter in line with the marks.
- **Q425 (lanes): closes as already stacked.** `.propblock + .propblock` stacks with a hairline (`system.css:1462`); the editing card's side-by-side pair was retired (`:2163`); nothing on a card is side by side except the commit row (two controls fit at 320px) and B · I · []. Asserted as no `.sugg` with `scrollWidth > clientWidth` at 360px.
- **Q427 (hairlines): a second named token, not a darker `--border`** (Q659). `--border` #DEE2E6 (~1.3:1) is fine for structural rules the layout already carries. The load-bearing edges — whose *absence* changes meaning: the incumbent's dashed edge in the sealed record, the `.qwhy + .qwhy` divider between quoted proposals, the outline ground of 🗑️ and Refuse (§9.1: outline is their whole look) — get `--border-strong` = `--muted` #6C757D (4.7:1), **its users named in the selector** as `.eyebrow` names its (`system.css:212`). Surface-wide: one edge, one contrast; geometry-neutral (probe-safe) but visibly darker on ~5 desktop edges. Measured in §2.1 #9.

### 1.3 The four passes' narrow behaviour, named

- **`layoutQueue`** — extract the classification (`anchorForEntry`, `session.js:958`; the pinned/flow split inside `layoutQueue`, :1014) into `classifyEntries()` shared by both layouts; if `SESSION.narrow()`, `layoutQueueNarrow()`: flow entries `display:none`, the same `keep` loop with `room` = dock capacity, kept entries stacked in the dock with `QGAP`, `--dock-h` set on the root (the alpha flag and `bringIntoView` read it), `.offclause` judged against `[navH, innerHeight − dockH]` so M9 holds unchanged.
- **`drawWires`** — if narrow, clear the SVG and return (`system.css:2091` stays as belt-and-braces). A wire from a dock to a clause anywhere on the page says nothing.
...
- **`bringIntoView` / `readLine()`** — `READ_LINE` becomes mode-dependent: wide 150, narrow `navH + 32`; the *already comfortable* window `[100, 300]` becomes `[navH + 8, innerHeight − dockH − 120]`. `readLine()` is already exported (`session.js:173`, over `READ_LINE` at :3955) so setup's `scrollToCard` follows for free.
...
- **On coarse pointers the charter's `contenteditable` is off** — set by `bindData`/`renderDoc`, not CSS — so a tap places no caret and raises no `beforeinput`; the `MAY_PROPOSE()` refusal (`session.js:152`) is the fallback. This is the seam for Q424 (§1.7).
...
- `100vh` → `100dvh` at `system.css:374` (the `vh` line kept as fallback). The `.doc` 95vh runway stays `vh` deliberately.
...
**Replace, not extend** (lines as re-checked 2026-09-07). `:358` (1240) stays. `:362` rewritten (one column; `.queue` fixed dock; `.toc` drawer — and its `display: flex` line, which would *un-hide* the TOC above the document, goes). `:870` rewritten (gutter 56, no border). `:940` deleted. `:1145` deleted (the rule that breaks both fitting passes). `:2091` kept plus the JS early return.

**Desktop-invisible by construction**: anything inside `(max-width: 900px)`, `(pointer: coarse)`, `(hover: none)`, or gated on `SESSION.narrow()`/`coarse()`/`pointerType === 'touch'`. The probes run at 1600×1000 with a fine pointer and hover; `matchMedia` is stubbed only for reduced motion (`session-probe.js:35`), so `NARROW_Q` reports false.
...
Where each stage lands. Lines checked 2026-08-23 and re-checked 2026-09-07; the stages that move them say so.
...
| `packages/server/src/server.ts` | `serveFile` (:1827) and `MIME` (:1815) for caching and `.webmanifest`; the static-asset gate (:1560–1610); `relay()` (:199) as push's second queue; the view route (:1374–1404) | 5 |
| `packages/server/src/persistence.ts`, `pg-persistence.ts` | `push_subscriptions` in the next migration (v5) and its `push.jsonl` mirror; put/delete/list/deleteFor on both stores (Q673) | 5 |
