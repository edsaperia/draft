...
(function () {
  ...
  function editCardHtml(d, site) {
    ...
    return (
      ...
      (broke ? ' disabled title="No edits left — another arrives as the drip accrues"' : '') +
  ...
  function suggCardHtml(s, siteKey) {
    ...
    if (s.kind === 'race') {
      ...
      return (
        ...
        // explaining itself.
        '<div class="foot">Neither of these has to win — the clause above stands unless the leader clears the bar.</div>' +
        commitRowHtml(s) +
        '</div>'
      );
    }
  ...
  function renderWallet(showAs) {
    ...
    walletEl.title = held === 0
      ? 'No edits left. Another arrives as the drip accrues; proposing costs one.'
      : 'Your edits — proposing one costs ' + EDIT_RULES.stake +
