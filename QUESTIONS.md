...
48. **The keep-current verdict on rival cards** (from race-card design, 2026-08-14): the UI now always offers "Keep the current text", even when the served pair is challenger-vs-challenger. Proposed mechanism: that gesture records two comparisons (incumbent beats A, incumbent beats B) — Davidson handles arbitrary pairs, so it's statistically clean — but it mints two edges from one act of attention, which slightly overweights status-quo defenders in evidence volume. Options: (a) record both edges; (b) record both at half weight (spec currently says judgments count once each, no weights — would need amending); (c) record only one edge, against the option the judge was more nearly tempted by (extra prompt, friction). Recommendation: (a), monitored in sim for status-quo bias.
...
Next unused number: 50 (30, 33-41, 44-47 were ephemeral chat items, 2026-08-13/14; 46 — diagonal-card visual distinction, likely a colour tint — is parked to decide with the palette work).
