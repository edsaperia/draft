...
29. **Off-menu welfare scoring** (asked 2026-08-13): LLM-persona runs end with nearly every adopted line off the scenario's known-alternatives menu, so the welfare ratio falls back to incumbent utility and reads 0.00 — meaningless, not bad. Options: (a) LLM-judge scoring — rate each final line against each persona's latent stance/salience to synthesize comparable utilities (adds cost and noise); (b) nearest-menu mapping — embed final text, snap to closest known alternative, score that (cheap, crude); (c) accept it — welfare is a scripted-run metric, LLM runs are judged qualitatively (current de facto state, now labeled). Recommendation: (c) for now, (a) when calibration begins.
