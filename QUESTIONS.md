...
49. **Naming and typing disputes** (Ed's question, 2026-08-14): the engine knows a dispute's *location* (span/footprint), not its *nature*. Labels in UI currently fall back to nearest markdown heading + excerpt, which fails on heading-less documents. P3 should generate both a name ("treasurer oversight") and a type (copy-edit vs substantive vs structural) per race — the type also informs routing (copy-edits shouldn't burn diagonal attention slots) and the record. Until P3: nearest-heading + excerpt.

50. **Revising judgments** (Ed's question, 2026-08-14): judgments are currently append-only and a judged pair is never re-served to the same participant. But a judgment is implicitly conditional on the document state it was made in — couplings make this literal (your honest guests preference can flip when keys formalises) — and the engine already stales incumbent-involving evidence when the ground changes (incumbent hash), while challenger-vs-challenger edges survive ground shifts unstaled. Options: (a) judgments are final (v1 status quo — simplest, gaming-proof); (b) ground-shift re-serving — when a race's span or coupled context has materially changed, its judged pairs become re-servable and a new judgment supersedes the old (event-sourced 'superseded'; ranking uses latest); (c) free revision from history at any time (risk: late strategic flips when the bar is high, plus peakW/refund interactions). Recommendation: (b) — principled and bounded; sim can A/B it.
...
Next unused number: 51 (30, 33–41, 44–47 were ephemeral chat items, 2026-08-13/14; 46 is parked — diagonal-card visual distinction, decide with the palette work; 48 resolved 2026-08-14, folded into SPEC §8.3).
