...
| 9 | Resend domain and deliverability (433) | done 2026-08-20 | DNS confirmed by query (DKIM at `resend._domainkey.mail.docs.vote`, SPF and MX at `send.mail.docs.vote`); the domain verified and the sandbox sender cleared by Ed (running log, 23:15); `config.ts:199` defaults the sender to `docs.vote <invitations@mail.docs.vote>`; OPERATING §1. The one real send to a non-Resend address the stage asked for was never recorded — see *Questions* in the pass log |
| 10 | docs.vote live (481 (a)); security review #2 | done 2026-08-20 | certificate issued, `www` and http 301, `npm run verify` 10/10 on the host; review #2's first pass `c8a732d` (2026-08-20) and its pass over the stranger's door (`083d95e`, 2026-08-21 — stored XSS at the public door, pinned by a server test). Still owed: stage 19's targeted review of the seams changed since |
